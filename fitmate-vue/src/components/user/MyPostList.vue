...
const search = async function () {
  await store.searchPostListByMe(searchCondition.value, userStore.loginUser.id);
};
...
const totalPages = computed(() =>
  Math.ceil(filteredPostList.value.length / postsPerPage)
);
...
const paginatedPosts = computed(() =>
  filteredPostList.value.slice(
    startIndex.value,
    startIndex.value + postsPerPage
  )
);
const filteredPostList = computed(() => {
  let postList = store.postList;

  postList = postList.filter((post) => userStore.loginUser.id === post.writer);
